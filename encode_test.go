...
func TestEncode(t *testing.T) {
	...
	t.Run("squashed fields are inlined, not nested", func(t *testing.T) {
		got := marshal(t, fixture{Name: "x", FixtureBase: FixtureBase{Kept: "yes"}})
	...
	t.Run("a duration is written in its String form", func(t *testing.T) {
		got := marshal(t, fixture{Name: "x", Timeout: 90 * time.Second})
...
func marshalAny(v any) ([]byte, error) {
	...
	if err := (Marshaler{}).encodeValue(enc, reflect.ValueOf(v)); err != nil {
		return nil, err
	}
